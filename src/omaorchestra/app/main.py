...
def run(check=False, session=""):
    ...
    away_mode = backend.Away(sessions)
    session_history = backend.History(sessions)
    ...
    ctx.setContextProperty("awayMode", away_mode)
    ctx.setContextProperty("sessionHistory", session_history)
